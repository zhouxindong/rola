#ifndef ROLA_ATM_ATM_BUSI_HPP
#define ROLA_ATM_ATM_BUSI_HPP
...
namespace rola
{
	...
	{
		// atm state machine
		// atm business logic class
		class atm
		{
			csp::receiver incoming; // a receiver, own a message queue
			csp::sender bank;
			csp::sender interface_hardware;

			void (atm::* state)(); // current atm state
			std::string account;
		...
		public:
			atm(csp::sender bank_, csp::sender interface_hardware_)
				: bank(bank_)
