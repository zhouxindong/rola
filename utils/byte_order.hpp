...
#pragma region byte order defines
	...
	inline std::ostream& operator<<(std::ostream& out, ByteOrder bo)
	{
		switch (bo)
		{
		case ByteOrder::BO_UNKNOWN:
			out << "ByteOrder:UNKNOWN";
			break;

		case ByteOrder::BO_BIG_ENDIAN:
			out << "ByteOrder::ENDIAN";
			break;
...
#pragma region Integer_for_size
...
#pragma region reverse byte order
	...
	{
		return reverse_float_helper(source);
	}
...
#pragma region big/little endian
	...
	{
		rola::ByteOrder bo = rola::Order_detector::get_order();
		if (bo == rola::ByteOrder::BO_BIG_ENDIAN)
			return source;
		else if (bo == rola::ByteOrder::BO_LITTLE_ENDIAN)
			return rola::reverse_byteorder(source);

		throw std::runtime_error("unknown platform byte order");
	}
