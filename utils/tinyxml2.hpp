...
#pragma endregion // tinyxml2

#pragma region Easy_tinyxml2
...
// parse the xml file encoded by utf8(no bom)
class Easy_tinyxml2
{
	std::vector<XMLElement*> vec_;
	XMLDocument* doc_{ nullptr };
	bool ready_{ false };

public:
	Easy_tinyxml2(std::string const& file)
	{
		doc_ = new XMLDocument();
		ready_ = XML_SUCCESS == doc_->LoadFile(file.c_str());
	}

	~Easy_tinyxml2() noexcept
	{
		if (doc_ != nullptr)
			delete doc_;
	}

	Easy_tinyxml2(Easy_tinyxml2 const&) = delete;
	Easy_tinyxml2(Easy_tinyxml2&&) = delete;
	Easy_tinyxml2& operator=(Easy_tinyxml2 const&) = delete;
	Easy_tinyxml2& operator=(Easy_tinyxml2&&) = delete;
...
public:
	...
	bool has_ancestry(XMLElement* ele, std::string const& ancestry) const
	{
		auto p_node = ele->Parent();
		while (true)
		{
			if (ancestry == p_node->ToElement()->Name())
				return true;
...
public:
	std::vector<XMLElement*> all(std::string const& ele_name)
	{
		ASSERT_ANY(ready_);
		vec_.clear();
		all_(ele_name, root());
	...
	std::vector<XMLElement*> all(std::string const& ele_name, std::string const& parent)
	{
		ASSERT_ANY(ready_);
		vec_.clear();
	...
	std::vector<XMLElement*> all(std::string const& ele_name, std::string const& parent, std::string const& pparent)
	{
		ASSERT_ANY(ready_);
		vec_.clear();
	...
	std::vector<XMLElement*> childs(XMLElement* ele, char const* child_name = nullptr) const
	{
		std::vector<XMLElement*> v;
		if (ele->NoChildren())
			return v;

		XMLElement* n = ele->FirstChildElement(child_name);
		while (n)
		{
			v.push_back(n);
	...
	XMLElement* parent(XMLElement* ele) const
	{
		return ele->Parent()->ToElement();
	}

public:
	std::string text(XMLElement* ele) const
	{
		return ele->GetText();
	}

	std::pair<bool, int32_t> int32_attribute(XMLElement* ele, char const* attr_name)
	{
		int32_t value;
	...
	std::pair<bool, std::string> string_attribute(XMLElement* ele, char const* attr_name)
	{
		auto v = ele->Attribute(attr_name);
		return std::make_pair(v != NULL, v == NULL ? "" : v);
	}

	std::pair<bool, bool> bool_attribute(XMLElement* ele, char const* attr_name)
	{
		bool value;
	...
	std::pair<bool, double> double_attribute(XMLElement* ele, char const* attr_name)
	{
		double value;
...
private:
	void all_(std::string const& name, XMLElement* ele)
	{
		if (name == ele->Name()) {
			vec_.push_back(ele);
		}
	...
	void all_(std::string const& name, XMLElement* ele, std::string const& parent)
	{
		if (name == ele->Name()) {
			if (ele->Parent() &&
				(parent == ele->Parent()->ToElement()->Name()))
				vec_.push_back(ele);
		}
		if (ele->FirstChildElement()) {
	...
	void all_(std::string const& name, XMLElement* ele, std::string const& parent, std::string const& pparent)
	{
		if (name == ele->Name()) {
			if (ele->Parent() &&
				(parent == ele->Parent()->ToElement()->Name()))
				if (ele->Parent()->Parent() &&
					(pparent == ele->Parent()->Parent()->ToElement()->Name()))
					vec_.push_back(ele);
		}
		if (ele->FirstChildElement()) {
...
#pragma endregion // Easy_tinyxml2
