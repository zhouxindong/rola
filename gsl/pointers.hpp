...
#include "assert.hpp"	// for Ensures, Expects
...
namespace gsl
{
	...
	public:
		...
		constexpr not_null(std::unique_ptr<std::remove_pointer_t<T>>& up) : not_null(up.get())
		{}

		constexpr not_null(std::shared_ptr<std::remove_pointer_t<T>>& up) : not_null(up.get())
		{}
...
namespace gsl
{
	...
	To* safe_reinterpret_cast(From* p)
	{
		void* tmp = p;
